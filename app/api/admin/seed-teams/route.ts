...
const logger = createLogger("admin-seed-teams");
...
export async function POST() {
  try {
    logger.info("Seeding 30 MLB teams...");
    ...
    logger.info("Teams seeded successfully", { created: result.count });

    return NextResponse.json({
      message: "Teams seeded successfully",
      created: result.count,
      total: mlbTeams.length,
    });
  } catch (error) {
